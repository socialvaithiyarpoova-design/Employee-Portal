...
function AddToCart() {
  ...
  const fromPage = location.pathname.includes("/appointments")
  ? "appointments"
  : "leads";
  ...
  useEffect(() => {
    if (rowData?.lead_recid) {
      axios
        .get(`${config.apiBaseUrl}getWalletByLead/${rowData.lead_recid}`)
        .then((res) => {
  ...
  const basePath =
    fromPage === "appointments"
      ? "/appointments/add-to-card/order-form"
      : "/leads/add-to-card/order-form"; 
