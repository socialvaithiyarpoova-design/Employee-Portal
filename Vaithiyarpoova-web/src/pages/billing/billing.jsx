...
function Billing() {
  ...
  // MODIFIED: Now stores product metadata when quantity changes
  const handleQuantityChange = (productId, delta) => {
    setQuantities((prev) => {
      ...
      if (newQty > 0) {
        const product = productList.find(p => p.stock_product_id === productId);
        if (product) {
          setProductMetadata((prevMeta) => ({
            ...prevMeta,
            [productId]: {
              rec_id: product.stock_recid,
  ...
  const getAllProducts = async () => {
    ...
    try {
      const response = await axios.post(
        `${config.apiBaseUrl}getallproductsbilling`,
         {
    ...
    <div className="common-body-st">
      ...
      <div className="header-cart-st" style={{padding:'16px'}}>
        ...
        <div className="rightcorn-ts-st">
         ...
         <button type="button " className="btn-proceed-st" style={{padding:'7.5px 15px'}}  onClick={() => navigate("/billing/history")}>
           History
         </button>
        </div>
      </div>

      <div className="body-div-el p-0">
        <div className="container-fluid overflow-auto d-flex gap-3 p-3 w-100 h-100">
          ...
          >
            ...
            <div className="w-25 display-flex text-center p-2 pt-3 pb-3">
              <div className="d-flex justify-content-center align-items-center">
                <button
                  ...
                  onClick={() => handleQuantityChange(product.stock_product_id, -1)}
                >
                  -
                </button>
                <input
                  type="text"
                  value={quantities[product.stock_product_id] ?? 0}
                  readOnly
            ...
            <div className="w-25 display-flex text-center p-2 pt-3 pb-3">
              {quantities[product.stock_product_id] > 0
                ? `₹ ${quantities[product.stock_product_id] * product.selling_price}`
                : "₹ 0"}
            </div>
          </div>
            ))}
