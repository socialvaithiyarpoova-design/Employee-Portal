...
--     IN p_courier_amount DECIMAL(10,2),
...
--     IN p_user_id INT(11),
--     IN p_cat_id VARCHAR(250),
--     IN p_qty INT(11),
--     IN rec_id VARCHAR(1000),
--     IN stick_type VARCHAR(50),
--     IN p_products_data JSON  ,
--     IN p_gst_id INT(11)
-- )
-- BEGIN
...
--     INTO next_order_num
--     FROM sales
--     WHERE order_id LIKE 'VPO%';

--     SET new_order_id = CONCAT('VPO', LPAD(next_order_num, 4, '0'));
...
--         district, state, country,pincode, courier, category, order_value, discount,
--         approved_by, payment_mode,courier_amount, wallet,gst_amount, final_amount, total_value, amount_to_pay,
--         medication_period, receipt_image_url, transaction_id, date_time,dispatch_id,
--         created_by, order_name, quantity, Product_id, stick_type, product_list, gst_id
--     ) VALUES (
--         p_order_id, p_leads_id, p_direct_pickup, p_additional_number, p_address,
--         p_district, p_state, p_country,p_pincode, p_courier, p_category, p_order_value, p_discount,
--         p_approved_by, p_payment_mode,p_courier_amount, p_wallet,p_gst_amount, p_total_value, p_amount_to_pay, p_amount_to_pay,
--         p_medication_period, p_receipt_image_url, p_transaction_id, p_date_time,p_dispatch_id ,
--         p_user_id, p_cat_id, p_qty, rec_id, stick_type, p_products_data, p_gst_id
--     );

--     -- Get the last inserted order id
--     SET last_order_id = LAST_INSERT_ID();

--     -- Insert purchased products using JSON_TABLE
    ...
    IF p_transaction_id IS NOT NULL AND p_transaction_id <> '' THEN
        ...
        IF EXISTS (SELECT 1 FROM transaction_id_list WHERE transaction_id = p_transaction_id) THEN
            SIGNAL SQLSTATE '45000'
                SET MESSAGE_TEXT = 'Duplicate transaction_id - must be unique';
        END IF;

        -- Insert transaction id into tracking table
