DELIMITER //

DROP PROCEDURE IF EXISTS `SP_GetSaleRevenueByBranch`;

CREATE PROCEDURE `SP_GetSaleRevenueByBranch`(
    IN start_date VARCHAR(100),
    IN end_date VARCHAR(100),
    IN branch_id INT(11),
    IN employee_id INT(11),
    IN usertype_code VARCHAR(100),
    IN iUser_id INT(11)
)
BEGIN
    DECLARE sCondition TEXT DEFAULT '';
    DECLARE sBRCondition TEXT DEFAULT '';

    IF end_date IS NOT NULL AND start_date IS NOT NULL AND start_date <> '' AND end_date <> '' THEN 
        SET sCondition = CONCAT(" AND DATE(SL.date_time) BETWEEN '", start_date, "' AND '", end_date, "' "); 
    END IF;

    IF branch_id IS NOT NULL THEN
        SELECT branch_incharge_recid INTO @iBranch_id FROM branches WHERE branch_recid = branch_id;
        SET sBRCondition = CONCAT(sBRCondition, " AND USS.created_by = ", @iBranch_id); 
    END IF;
    
    IF employee_id IS NOT NULL THEN
        SELECT user_id INTO @iUserId FROM users WHERE user_id = employee_id;
        SET sBRCondition = CONCAT(sBRCondition, " AND USS.user_id = ", @iUserId); 
    END IF;

    IF usertype_code = "BH" THEN 
         SET sBRCondition = CONCAT(sBRCondition, " AND USS.created_by = ", iUser_id); 
    END IF;

    SET @sQuery = CONCAT("
        SELECT 
            DISTINCT user_recid AS user_recid,
            GROUP_CONCAT(DISTINCT user_types) AS user_types,
            GROUP_CONCAT(DISTINCT branch_id) AS branch_id,
            GROUP_CONCAT(DISTINCT branch_name) AS branch_name,
            GROUP_CONCAT(DISTINCT user_id) AS user_id,
            GROUP_CONCAT(DISTINCT emp_id) AS emp_id,
            SUM(total_amount) AS total_amount,
            GROUP_CONCAT(DISTINCT location) AS location
        FROM (
            /* sales table aggregated per created_by */
            SELECT 
                USS.created_by AS user_recid,
                GROUP_CONCAT(DISTINCT UST.designation) AS user_types,
                GROUP_CONCAT(DISTINCT UST.emp_id) AS branch_id,
                GROUP_CONCAT(DISTINCT UST.name) AS branch_name,
                GROUP_CONCAT(DISTINCT USS.user_id) AS user_id,
                GROUP_CONCAT(DISTINCT USS.emp_id) AS emp_id,
                ROUND(SUM(SL.final_amount),2) AS total_amount,
                GROUP_CONCAT(DISTINCT BR.locations) AS location
            FROM
                users AS USS
                LEFT JOIN sales AS SL ON SL.created_by = USS.user_id
                LEFT JOIN users AS UST ON UST.user_id = USS.created_by
                LEFT JOIN (
                    SELECT branch_incharge_recid,
                           GROUP_CONCAT(DISTINCT location) AS locations
                    FROM branches 
                    GROUP BY branch_incharge_recid
                ) AS BR ON BR.branch_incharge_recid = UST.created_by
            WHERE
                USS.created_by IN (
                    SELECT DISTINCT U.user_id
                    FROM users AS U
                    LEFT JOIN users AS US ON US.created_by = U.user_id
                    LEFT JOIN usertype AS UT ON UT.usertype_id = US.usertype_id
                    WHERE U.isDeleted = 0 AND US.isDeleted = 0 AND US.created_by IS NOT NULL
                )
                AND USS.isDeleted = 0
                AND SL.order_recid IS NOT NULL
                AND SL.status IN ('Approved', 'Dispatched', 'In transit') ",
                IFNULL(sCondition, ''), IFNULL(sBRCondition, ''), "
            GROUP BY USS.created_by

            UNION

            /* fieldshopsales aggregated per created_by */
            SELECT 
                USS.created_by AS user_recid,
                GROUP_CONCAT(DISTINCT UST.designation) AS user_types,
                GROUP_CONCAT(DISTINCT UST.emp_id) AS branch_id,
                GROUP_CONCAT(DISTINCT UST.name) AS branch_name,
                GROUP_CONCAT(DISTINCT USS.user_id) AS user_id,
                GROUP_CONCAT(DISTINCT USS.emp_id) AS emp_id,
                ROUND(SUM(SL.final_amount),2) AS total_amount,
                GROUP_CONCAT(DISTINCT BR.locations) AS location
            FROM
                users AS USS
                LEFT JOIN fieldshopsales AS SL ON SL.created_by = USS.user_id
                LEFT JOIN users AS UST ON UST.user_id = USS.created_by
                LEFT JOIN (
                    SELECT branch_incharge_recid,
                           GROUP_CONCAT(DISTINCT location) AS locations
                    FROM branches 
                    GROUP BY branch_incharge_recid
                ) AS BR ON BR.branch_incharge_recid = UST.created_by
            WHERE
                USS.created_by IN (
                    SELECT DISTINCT U.user_id
                    FROM users AS U
                    LEFT JOIN users AS US ON US.created_by = U.user_id
                    LEFT JOIN usertype AS UT ON UT.usertype_id = US.usertype_id
                    WHERE U.isDeleted = 0 AND US.isDeleted = 0 AND US.created_by IS NOT NULL
                )
                AND USS.isDeleted = 0
                AND SL.order_recid IS NOT NULL
                AND SL.status IN ('Approved', 'Dispatched', 'In transit') ",
                IFNULL(sCondition, ''), IFNULL(sBRCondition, ''), "
            GROUP BY USS.created_by
        ) AS A
        GROUP BY A.user_recid
    ");

    PREPARE stmt FROM @sQuery;
    EXECUTE stmt;
    DEALLOCATE PREPARE stmt;

END//

DELIMITER ;
