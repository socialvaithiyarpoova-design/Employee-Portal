...
DROP PROCEDURE IF EXISTS `SP_GetConsultRevenueByBranch`;

CREATE PROCEDURE `SP_GetConsultRevenueByBranch`(
...
BEGIN   
    ...
    IF end_date IS NOT NULL AND start_date IS NOT NULL AND start_date <> '' AND end_date <> '' THEN 
        SET sCondition = CONCAT(" AND DATE(CA.created_at) BETWEEN '",start_date,"' AND '",end_date,"' "); 
    END IF;
    ...
    SET @sQuery = CONCAT("
                ...
                FROM users AS USS
                LEFT JOIN consulting_appointments AS CA ON CA.created_by = USS.user_id
                ...
                WHERE USS.created_by IN (
                    ...
                    AND USS.isDeleted = 0 
                    AND CA.id IS NOT NULL  AND CA.status = 'Approved' ",IFNULL(sCondition, ''),"  
